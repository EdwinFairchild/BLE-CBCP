...
// TODO : make set get functions for these switches/varaibles

uint8_t peer_name_len = 4;
uint8_t peer_name[20] = "DIRT";
bool name_set = false;
...
static void datcPrintScanReport(dmEvt_t *pMsg)
{
    ...
    uint8_t *pData;
    if(printScan){
        APP_TRACE_INFO2("%s : %d: Scan Report:", peer_name, peer_name_len);
...
static void datcScanReport(dmEvt_t *pMsg)
{
    ...
    if (!connect && ((pData = DmFindAdType(DM_ADV_TYPE_LOCAL_NAME, pMsg->scanReport.len,
        ...
        //     connect = TRUE;
        //     APP_TRACE_INFO1("0------->IDX LEN is : %d", pData[DM_AD_LEN_IDX]);
        // }
        if (pData[DM_AD_LEN_IDX] >= peer_name_len && name_set == true) {
            uint8_t err = 0;
            for (int i = 0; i < 4; i++) {
                if (pData[DM_AD_DATA_IDX + i] != peer_name[i]) {
                    err++;
                }
            }
            if (err == 0)
